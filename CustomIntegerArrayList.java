...
public class CustomIntegerArrayList {
	...
	public void add(int index, int element) {
		arr.add(index, element);				// built-in `add` method knows its syntax
	}
	...
	public int remove(int index) {
		if (index >= 0 && index < arr.size()) {
			return arr.remove(index);				// built-in `remove` method knows its syntax (Py's `pop`)
		} else {
			throw new IndexOutOfBoundsException("No such index.");
		}
	}
	...
	public void remove(int num, int element) {
		if (num <= 0) {
			return;  // Don't remove anything if num <= 0
		}
															// while loop - to go through ALL but unknown number of elements.
		while (num > 0 && arr.contains(element)) {				// built-in `contains`.
			arr.remove(Integer.valueOf(element));				// can only remove objects, not primitives, so wrapper class Integer.
			num--;
		}
	}
	...
	public ArrayList<Integer> splice(int index, int num) {
		if (index < 0 || index >= arr.size() || num == 0) {
			...
			while (num > 0 && index < arr.size()) {		
				removed.add(arr.remove(index));			// add to new ArrayList what's been removed from original ArrayList
				num--;
			}
			return removed;
		}
	...
	public ArrayList<Integer> splice(int index, int num, int[] otherArray) {
		if (index < 0 || index >= arr.size() || num == 0) {
			...
			ArrayList<Integer> removedElems = new ArrayList<Integer>();
			while (num > 0 && index < arr.size()) {		
				removedElems.add(arr.remove(index));			// add to new ArrayList what's been removed from original ArrayList
				num--;
			}
			for (int i = 0; i < otherArray.length; i++) {
				arr.add(index, otherArray[i]);		// add elements from otherArray to original ArrayList
			}
			return removedElems;
		}
	}
